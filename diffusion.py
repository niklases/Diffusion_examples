...
# Niklas Siedhoff, 03 December 2024
# Code taken (and slightly adapted that enable importing multiple datasets and other added utilities) 
# from 
...
# If WSL2 is missing ptxas, one can install cuda-nvcc in conda:
...
## Hyperparameters
"""
for dir in ['checkpoints', 'images']:
    os.makedirs(
        os.path.join(os.path.dirname(__file__), dir), 
        exist_ok=True
    )

# User Settings: Training, Continue Training, or run Inference only ##############
training = True
save_freq = 10  # Saving checkpoint every N_Epochs, globally used in Tensorflow function
continue_training = False
continue_from_epoch = 100
inference = True
plot_freq = 10
flowers = False  # Flowers dataset; else: MNIST dataset
mnist = False
hf = True
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
batch_size = 10
...
def resize_augment(images, size=(32, 32)):
    images_processed = []
    for image in tqdm(images, desc="Resizing & augmenting images"):
        image = resize(image, size)
        image = augment(image)
        images_processed.append(image)
    return tf.convert_to_tensor(images_processed)


if flowers:  # FLOWERS ########################################################
    images = np.load('datasets/tensorflow/oxford_flowers102.npy')
    images_processed = resize_augment(images, size=(96, 96))
elif mnist:  # MNIST NUMBERS ##################################################
    images = np.load("datasets/mnist/mnist.npy")
    images_processed = resize_augment(images, size=(32,32))
elif hf:    # HF dataset ######################################################
    images = np.load('datasets/tensorflow/food101.npy')
    images_processed = resize_augment(images, size=(96, 96))
###############################################################################
print(f'Input shape (all images stacked): {np.shape(images_processed)}')  # e.g. (163, 500, 667, 3) -> 163 images of size 500 x 667 pixels each pixel being 3 (RGB) values
img_size = np.shape(images_processed)[1]
print(img_size)
fig, ax = plt.subplots()
ax.imshow(images_processed[42])
plt.show()
if images_processed.shape[0] % batch_size > 0:
    images_processed = images_processed[:-(images_processed.shape[0] % batch_size)]
...
class DiffusionModel(keras.Model):
    ...
    def save_ema_network_weigths(self, epoch=None, logs=None):
        """Method to be used in Callbacks: Utility to save weights every epoch.
        Accessing global variable `save_freq`.
        """
        self.n_completed_epochs += 1
        if self.n_completed_epochs % save_freq == 0:
            print(f'\nSaving weights of epoch {self.n_completed_epochs} (a save occurs every {save_freq} epochs) '
                  f'as checkpoints/cp_{self.n_completed_epochs:04d}.weights.h5')
            self.ema_network.save_weights(
                'checkpoints/cp_{epoch:04d}.weights.h5'.format(epoch=self.n_completed_epochs))
            self.network.save_weights(
                'checkpoints/cp_network_{epoch:04d}.weights.h5'.format(epoch=self.n_completed_epochs))

    def plot_images(
        self, epoch=None, logs=None, num_rows=2, num_cols=8, figsize=(12, 5), 
        save_to_disk: bool = False, save_path: None | str = None
    ):
        """Method to be used in Callbacks: Utility to plot images using the diffusion model during training."""
        ...
        if save_to_disk:
            print('Plotting...')
            if not save_path:
                file_path = os.path.join(os.path.dirname(__file__), 'Result.png')
                plt.savefig(file_path, dpi=300)
            else:
                file_path = os.path.join(os.path.dirname(__file__), save_path)
                plt.savefig(file_path, dpi=300)
            plt.clf()
            print(f'Plotted result as {file_path}...')
        else:
            plt.show()
...
model.ema_network.save_weights('checkpoints/cp_{epoch:04d}.weights.h5'.format(epoch=0))
model.network.save_weights('checkpoints/cp_network_{epoch:04d}.weights.h5'.format(epoch=0))
...
if training:
    ...
    m=model.fit(
        ...
        callbacks=[keras.callbacks.LambdaCallback(on_epoch_end=model.save_ema_network_weigths), history]
    )
    plt.figure(figsize=(60,10))
    plt.scatter(range(len(history.losses)), history.losses)
    n_batches_per_epoch = np.shape(images_processed)[0] // batch_size
    print(history.losses)
    for e in range(0, num_epochs):
        plt.plot([e * n_batches_per_epoch, (e+1) * n_batches_per_epoch], 
                 [
                     np.mean(history.losses[(e) * n_batches_per_epoch:(e+1) * n_batches_per_epoch]),
                     np.mean(history.losses[(e) * n_batches_per_epoch:(e+1) * n_batches_per_epoch])
                 ]
        )
        plt.text(((e + 1) * n_batches_per_epoch) - 2, 
                 np.mean(history.losses[(e) * n_batches_per_epoch:(e+1) * n_batches_per_epoch]), 
                 f"Epoch {e}: {np.mean(history.losses[(e) * n_batches_per_epoch:(e+1) * n_batches_per_epoch]):.3f}"
        )

    plt.plot()
    plt.scatter(range(len(m.history["loss"])), m.history["loss"])
    plt.savefig('1.png')

if continue_training:
    print(f"Continuing training from epoch {continue_from_epoch}...")
    model.ema_network.load_weights('checkpoints/cp_{epoch:04d}.weights.h5'.format(epoch=continue_from_epoch))
    model.network.load_weights('checkpoints/cp_network_{epoch:04d}.weights.h5'.format(epoch=continue_from_epoch))
    model.fit(
        images_processed,
        epochs=num_epochs,
        batch_size=batch_size,
        callbacks=[keras.callbacks.LambdaCallback(on_epoch_end=model.save_ema_network_weigths)]
    )
...
saved_images = []
if inference:
    for e in range(0, num_epochs + 1, plot_freq):
        if e % plot_freq == 0:
            # Load the model weights
            print(f'Loading epoch {e} weights...')
            # EMA network weights are enough for inference
            model.ema_network.load_weights('checkpoints/cp_{epoch:04d}.weights.h5'.format(epoch=e))
            # Generate and plot some samples
            print(f'Plotting result image...')
            model.plot_images(num_rows=4, num_cols=8, save_to_disk=True, save_path=f'images/Results_epoch_{e}.png')
            saved_images.append(f'images/Results_epoch_{e}.png')
else:
    for e in range(0, num_epochs + 1, plot_freq):
    #for e in np.geomspace(1, num_epochs + 1, num=100):
        #if int(e) % plot_freq == 0:
            saved_images.append(f'images/Results_epoch_{int(e)}.png')
print("Creating GIF...")
# GIF creation of plotted images
imgs_for_animation = []
for i, saved_image in enumerate(tqdm(saved_images)):
    img = Image.open(saved_image)
    draw = ImageDraw.Draw(img)
    draw.text((55, 10), f"Epoch {list(range(0, num_epochs + 1, save_freq))[i]}", (0, 0, 0))
    imgs_for_animation.append(img)
imgs_for_animation = iter(imgs_for_animation)
# Taken from: https://stackoverflow.com/a/57751793
with contextlib.ExitStack() as stack:
    # extract  first image from iterator
    img = next(imgs_for_animation)
    # https://pillow.readthedocs.io/en/stable/handbook/image-file-formats.html#gif
    img.save(fp='images/diffusion.gif', format='GIF', append_images=imgs_for_animation, 
